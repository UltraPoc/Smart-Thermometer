...
void loop() {
  ...
  if (millis() - time1 >= DELTA * 60000 && getTemp() != -127 && getTemp != 85 && temp[0] != 85 && temp[1] != 85 && temp[2] != 85 && temp[3] != 85)   //запись значений
  {
    moveMas(&data1[0], &data2[0], &tm[0]);
    data1[71] = time.day * 100 + time.month;
    data2[71] = time.year + 2000;
    tm[71] = time.Hours * 100 + time.minutes;
    saveMas(quadVal(temp[0], temp[1], temp[2], temp[3]), &data1[0], &data2[0], &tm[0]);
    time1 = millis();
